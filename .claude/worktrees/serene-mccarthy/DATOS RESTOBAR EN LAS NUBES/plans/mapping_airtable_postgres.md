...
| Sistema Origen | origin_system | VARCHAR(20) | VAPI/manual |
| Creado el | created_at | TIMESTAMP | Fecha de creación |
| Actualizado el | updated_at | TIMESTAMP | Fecha última actualización |
...
| Sistema Fuente | system_source | VARCHAR(20) | VAPI/manual |
| Workflow ID | workflow_id | VARCHAR(50) | ID del proceso |
| Creado el | created_at | TIMESTAMP | Fecha de creación |
...
1. **Airtable como fuente de verdad inicial**
    - El sistema FastAPI escribe directamente en Airtable
    - Un servicio de sincronización lee cambios de Airtable y replica en PostgreSQL
...
1. **PostgreSQL como fuente de verdad**
   - El sistema FastAPI escribe directamente en PostgreSQL
   - Un servicio de sincronización replica cambios en Airtable (dashboard)
...
|---------------|---------|----------------------|
| Duración Reserva | `Hora Fin - Hora Inicio` | Calculado en servicio |
...
| Días hasta Reserva | `DATETIME_DIFF(Fecha, TODAY())` | Calculado en servicio |

**Nota**: Estos campos calculados NO se almacenan en PostgreSQL, se calculan en tiempo real en el servicio o frontend.
