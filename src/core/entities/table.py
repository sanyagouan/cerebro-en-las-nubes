...
class TableZone(str, Enum):
    """Zonas principales del restaurante."""

    TERRAZA = "Terraza"
    INTERIOR = "Interior"
    BARRA = "Barra"
    # Otros valores de Airtable se mapean a estos
    OTRO = "Otro"  # Fallback para valores desconocidos


# Mapping de valores de Airtable a zonas válidas
def normalize_zone(raw_zone: str) -> str:
    """Normaliza valores de Ubicación de Airtable a zonas válidas."""
    if not raw_zone:
        return "Interior"

    zone_lower = raw_zone.lower().strip()

    if "terraza" in zone_lower:
        return "Terraza"
    elif "barra" in zone_lower:
        return "Barra"
    elif "interior" in zone_lower:
        return "Interior"
    elif "baño" in zone_lower or "ventana" in zone_lower or "entrada" in zone_lower:
        # Ubicaciones especiales dentro del interior
        return "Interior"
    else:
        # Default a Interior para valores desconocidos
        return "Interior"
...
class Table(BaseModel):
    ...
    auxiliar_requerida: Optional[str] = Field(
        None, description="ID de la mesa auxiliar necesaria"
    )
    capacidad_ampliada: Optional[int] = Field(
        None, description="Capacidad máxima con auxiliar"
    )

    # Notas y restricciones
    notas: Optional[str] = Field(
        None, description="Avisos obligatorios (ej: 'Junto al baño')"
    )
...
MESAS_INTERIOR: List[dict] = [
    ...
    },
    {
        ...
        "prioridad": 7,
        "ampliable": True,
        "auxiliar_requerida": "AUX-4",
        "capacidad_ampliada": 8,
    },
    # Mesas de 8 personas
    {
        "id": "C8-A",
