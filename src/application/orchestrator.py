...
class Orchestrator:
    ...
    async def process_message(
        ...
        if intent == "confirmation":
            ...
            # Use special_request if extracted by Router, else use full message
            note_content = routing.get("special_request") or message
            logic_res = self.logic.update_notes_by_phone(client_phone, note_content)
            result["response"] = logic_res.get("message")
            return result

        elif intent == "modify_reservation":
            # Client wants to change their reservation (time, date, pax)
            new_time = routing.get("new_time")
            new_date = routing.get("new_date")
            new_pax = routing.get("new_pax")

            logic_res = self.logic.modify_booking_by_phone(
                client_phone, new_date=new_date, new_time=new_time, new_pax=new_pax
            )
            result["response"] = logic_res.get("message")
            return result
        ...
        if intent == "reservation":
            # Extract booking details from RouterAgent's extraction (routing dict)
            # Router extracts: new_date, new_time, new_pax, special_request
            extracted_date = routing.get("new_date", "")
            extracted_time = routing.get("new_time", "")
            ...
            logic_result = await self.logic.process(
                {
                    "date": extracted_date or metadata.get("date", ""),
                    "time": extracted_time or metadata.get("time", ""),
                    "pax": extracted_pax or metadata.get("pax", 2),
                    "client_name": metadata.get("client_name", ""),
                    "client_phone": client_phone,
                    "action": "create_reservation"
                    if _is_confirmation_intent(message)
            ...
            # === HANDLE CLOSED RESTAURANT ===
            if logic_result.get("error") == "closed":
                result["response"] = logic_result.get(
                    "message", "Lo siento, en ese horario estamos cerrados."
                )
                return result
            ...
            if logic_result.get("available"):
                if logic_result.get("booking_created"):
                    ...
                    result["response"] = (
                        f"¡Perfecto! Hemos pre-reservado su mesa. Le acabo de enviar un WhatsApp con los detalles. Por favor, responda SÍ a ese mensaje para confirmar definitivamente."
                    )
                else:
            ...
            human_result = await self.human.process(
                {"situation": "faq", "data": {"question": message}}
            )
            result["response"] = human_result.get("response", "")
            ...
            # Generic response
            human_result = await self.human.process(
                {"situation": "generic", "data": {"message": message}}
            )
            result["response"] = human_result.get(
                "response", "Lo siento, no he entendido bien."
            )
