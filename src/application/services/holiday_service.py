...
class HolidayService:
    ...
    Los festivos se cargan de Airtable y se complementan con reglas fijas.
    """
    ...
    def _load_holidays(self):
        """Carga los festivos de Airtable y reglas fijas."""
        # 1. Cargar reglas fijas y calculadas para el año actual y siguiente
        for y in [self.year, self.year + 1]:
            self._holidays[y] = self._get_holidays_for_year(y)
        
        # 2. Cargar desde Airtable (sobrescribe o añade)
        self._load_from_airtable()

    def _load_from_airtable(self):
        """Consulta la tabla Festivos de Airtable."""
        ...
        try:
            api = Api(api_key)
            table = api.table(BASE_ID, TABLES.get("FESTIVOS", "Festivos"))
            records = table.all()
            
            for record in records:
                fields = record.get("fields", {})
                fecha_str = fields.get("Fecha")
                if not fecha_str: continue
                
                fecha = date.fromisoformat(fecha_str)
                holiday = Holiday(
                    fecha=fecha,
                    nombre=fields.get("Nombre", "Festivo"),
                    tipo=HolidayType(fields.get("Tipo", "Local")),
                    es_alta_demanda=fields.get("Alta_Demanda", False)
                )
                
                # Añadir al mapa (reemplazando si ya existe por fecha)
                y = fecha.year
                if y not in self._holidays:
                    self._holidays[y] = []
                
                # Evitar duplicados por fecha
                self._holidays[y] = [h for h in self._holidays[y] if h.fecha != fecha]
                self._holidays[y].append(holiday)
                
        except Exception as e:
            # Fallback silencioso a hardcoded si falla Airtable
            pass
