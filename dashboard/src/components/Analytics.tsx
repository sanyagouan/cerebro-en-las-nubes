...
export default function Analytics() {
  ...
  const tableOccupancy = useMemo(() => {
    ...
    const ocupadas = tablesData.filter((t: any) => t.estado === 'Ocupada').length;
    const reservadas = tablesData.filter((t: any) => t.estado === 'Reservada').length;
    const libres = tablesData.filter((t: any) => t.estado === 'Libre').length;
    ...
    <div className="space-y-6">
      ...
      {toast && (
        <div className="fixed bottom-4 right-4 z-50 animate-slide-up">
          <div
            className={`flex items-center gap-3 px-6 py-4 rounded-lg shadow-lg ${toast.type === 'success' ? 'bg-green-600 text-white' : 'bg-red-600 text-white'
              }`}
          >
            {toast.type === 'success' ? (
