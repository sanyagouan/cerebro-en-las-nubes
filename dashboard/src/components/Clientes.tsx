...
function ClienteDetalleModal({ isOpen, onClose, clienteId, onToast }: ClienteDetalleModalProps) {
  ...
  const { data: reservationsData, isLoading: loadingReservations } = useCustomerReservations(clienteId || '');
  ...
  const handleAddPreference = async () => {
    ...
    try {
      await addPreferenceMutation.mutateAsync({
        ...
        descripcion: newPreferenceValue,
      });
  ...
  const handleDeletePreference = async (preferenceId: string) => {
    try {
      await deletePreferenceMutation.mutateAsync({ customerId: clienteId, preferenceId: preferenceId });
  ...
  const handleDeleteNote = async (noteId: string) => {
    try {
      await deleteNoteMutation.mutateAsync({ customerId: clienteId, noteId: noteId });
  ...
  const handleToggleNoteImportance = async (noteId: string) => {
    try {
      ...
      await toggleNoteImportanceMutation.mutateAsync({ customerId: clienteId, noteId: noteId, is_important: !noteToToggle?.is_important });
    } catch (error) {
  ...
  const handleChangeTier = async (newTier: CustomerTier) => {
    try {
      await updateTierMutation.mutateAsync({ customerId: clienteId, tier: newTier });
                    ...
                    {reservations.slice(0, 10).map((reserva) => (
                      <tr key={reserva.id} className="border-t border-gray-200">
                        ...
                        <td className="px-4 py-2">
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${reserva.estado === 'Completada'
                    ...
                    <div key={pref.id} className="flex items-center justify-between bg-white rounded-lg p-3 border border-gray-200">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="font-medium text-gray-900">{pref.descripcion}</p>
                        </div>
                      </div>
                      <button
            ...
            {loadingNotes ? (
              ...
              <div className="space-y-3">
                {notes.map((note) => (
                  <div
                    key={note.id}
                    className={`bg-white rounded-lg p-4 border-2 ${note.is_important ? 'border-yellow-300 bg-yellow-50/50' : 'border-gray-200'
                      }`}
                  >
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <p className="text-xs text-gray-500">
                            {new Date(note.created_at).toLocaleString('es-ES')} • {note.staff_user_name}
                          </p>
                          {note.is_important && (
    ...
    <div className="space-y-6">
      ...
      <div className="fixed top-4 right-4 z-50 space-y-2">
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className={`px-4 py-3 rounded-lg shadow-lg text-white animate-fade-in ${toast.type === 'success' ? 'bg-green-600' : toast.type === 'error' ? 'bg-red-600' : 'bg-blue-600'
              }`}
          >
            {toast.message}
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-4">
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 border rounded-lg transition-colors ${hasFilters
