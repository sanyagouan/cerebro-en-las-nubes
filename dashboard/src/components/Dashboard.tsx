...
function getEstadoBadgeClasses(estado: Reservation['estado']): string {
  switch (estado) {
    case 'Confirmada':
...
export default function Dashboard() {
  ...
  const reservas = reservasData?.reservations || [];
  ...
  const stats = useMemo(() => {
    const reservasHoy = reservas.filter((r: Reservation) => isToday(parseISO(r.fecha)));

    return {
      totalReservasHoy: reservasHoy.length,
      reservasConfirmadas: reservasHoy.filter((r: Reservation) => r.estado === 'Confirmada').length,
      reservasPendientes: reservasHoy.filter((r: Reservation) => r.estado === 'Pendiente').length,
      ocupacion: reservasHoy.length > 0
        ? Math.round((reservasHoy.filter((r: Reservation) => r.estado === 'Sentada' || r.estado === 'Completada').length / reservasHoy.length) * 100)
        : 0,
      clientesHoy: reservasHoy.reduce((acc, r: Reservation) => acc + (r.pax || 0), 0),
    };
  }, [reservas]);
