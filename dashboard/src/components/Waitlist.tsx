...
import { Clock, Users, Phone, MapPin, Bell, Trash2, Plus, Search, Filter } from 'lucide-react';
...
export default function Waitlist() {
  ...
  const filteredEntries = entries.filter((entry) => {
    ...
    return matchesSearch && matchesZone && entry.estado === 'waiting';
  });
                    ...
                    <tr key={entry.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-1.5 text-slate-700">
                          ...
                          <span className="font-medium">{entry.pax}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-sm font-medium border ${entry.zona_preferida ? ZONA_COLORS[entry.zona_preferida] : ''}`}>
                          <MapPin className="w-3.5 h-3.5" />
                          {entry.zona_preferida || 'Sin zona'}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-1.5 text-slate-700">
                          <Clock className="w-4 h-4 text-slate-400" />
                          <span className="font-medium">{formatWaitTime(entry.tiempo_espera_estimado)}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex px-3 py-1 rounded-lg text-sm font-medium border ${entry.estado ? STATUS_STYLES[entry.estado] : ''}`}>
                          {entry.estado ? STATUS_LABELS[entry.estado] : ''}
                        </span>
                      </td>
                      <td className="px-6 py-4">
