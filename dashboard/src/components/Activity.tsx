...
export default function Activity() {
  ...
  const filteredEvents = data?.events.filter((event) => {
    const matchesSearch =
      ...
      (event.usuario || 'Sistema').toLowerCase().includes(searchQuery.toLowerCase());

    const matchesEventType =
      selectedEventType === 'all' || event.tipo === selectedEventType;

    const matchesUser = selectedUser === 'all' || (event.usuario || 'Sistema') === selectedUser;
  ...
  const uniqueUsers = Array.from(
    new Set(data?.events.map((e) => e.usuario || 'Sistema') || [])
  ).sort();
          ...
          <div className="bg-white/80 backdrop-blur-sm border border-slate-200/60 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900">
                  {data?.events.filter((e) => e.tipo === 'reserva_creada')
                    .length || 0}
                </p>
              </div>
            </div>
          ...
          <div className="bg-white/80 backdrop-blur-sm border border-slate-200/60 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900">
                  {data?.events.filter((e) => e.tipo === 'cliente_sentado')
                    .length || 0}
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="relative">
              ...
              >
                ...
                <option value="reserva_creada">Reservas Creadas</option>
                <option value="reserva_editada">
                  Reservas Actualizadas
                </option>
                <option value="reserva_cancelada">
                  Reservas Canceladas
                </option>
                <option value="mesa_asignada">Mesas Asignadas</option>
                <option value="cliente_sentado">Clientes Sentados</option>
              </select>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="divide-y divide-slate-100">
              {filteredEvents.map((event) => {
                const config =
                  EVENT_TYPE_CONFIG[
                  event.tipo as keyof typeof EVENT_TYPE_CONFIG
                  ] || {
                    icon: AlertCircle,
                    label: event.tipo,
