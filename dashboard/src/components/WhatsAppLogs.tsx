...
export default function WhatsAppLogs() {
  ...
  const filteredLogs = useMemo(() => {
    ...
    return logs.messages.filter((message: WhatsAppMessage) => {
  ...
  const handleExportCSV = () => {
    ...
    const rows = filteredLogs.map((msg: WhatsAppMessage) => [
      msg.id,
      msg.phone_number,
      msg.message_type,
    ...
    <div className="space-y-8">
      ...
      <div className="fixed top-4 right-4 z-50 space-y-2">
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className={`px-4 py-3 rounded-lg shadow-lg text-white animate-fade-in ${toast.type === 'success' ? 'bg-green-600' : 'bg-red-600'
              }`}
          >
            {toast.message}
          </div>
      ...
      {showTemplates && (
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="space-y-4">
            {templates?.templates.map((template: MessageTemplate) => (
              <div
                key={template.id}
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div>
                    ...
                    <span
                      className={`inline-block px-2 py-1 rounded text-xs font-medium ${getTypeBadge(template.type).bg
              ...
              {(['all', 'sent', 'delivered', 'read', 'failed'] as const).map((status) => {
                const count =
                  status === 'all'
                    ...
                    : logs?.messages.filter((m: WhatsAppMessage) => m.status === status).length || 0;
              ...
              {(['all', 'confirmation', 'reminder', 'cancellation', 'waitlist'] as const).map(
                (type) => {
                  const count =
                    type === 'all'
                      ? logs?.messages.length || 0
                      : logs?.messages.filter((m: WhatsAppMessage) => m.message_type === type).length || 0;
          ...
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredLogs.length === 0 ? (
                ...
                filteredLogs.map((message: WhatsAppMessage) => {
