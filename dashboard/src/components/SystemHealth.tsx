...
export default function SystemHealth() {
  ...
  const services: any = healthData?.services || {};
  const metrics: any = metricsData || {};
  const uptime: any = uptimeData || {};
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {services && Object.entries(services).map(([key, service]: [string, any]) => {
              const config = SERVICE_CONFIG[key as keyof typeof SERVICE_CONFIG];
              const safeStatus = (service.status as keyof typeof HEALTH_STATUS_CONFIG) || 'unhealthy';
              const statusConfig = HEALTH_STATUS_CONFIG[safeStatus];
