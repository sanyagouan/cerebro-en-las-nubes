    ...
    <div className="space-y-6">
      ...
      <div className="fixed top-4 right-4 z-50 space-y-2">
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className={`px-4 py-3 rounded-lg shadow-lg text-white animate-fade-in flex items-center gap-2 ${toast.type === 'success'
              ? 'bg-green-600'
              : toast.type === 'error'
                ? 'bg-red-600'
                : 'bg-blue-600'
      ...
      <div className="flex gap-2 border-b border-gray-200">
        {tabs.map(({ key, label, icon: Icon }) => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-3 border-b-2 transition-colors ${activeTab === key
          ...
          onConfirm={async () => {
            try {
              if (deleteTarget.type === 'holiday') {
                ...
                showToast('Festivo eliminado exitosamente', 'success');
              } else {
...
function FestivosSection({ onEdit, onDelete, onCreateNew }: FestivosSectionProps) {
            ...
            >
              <div className="flex-1">
                <div className="flex items-center gap-3">
                  ...
                  <span
                    className={`px-2 py-1 rounded-full text-xs font-medium ${holiday.is_closed
